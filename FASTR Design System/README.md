...
The repo ships a small, opinionated **PNG-first workshop icon set** in `assets/icons/`:

`lecture.png`, `communication.png`, `debrief.png`, `coffee.png`, `lunch.png`, `globe.png`, `email.png`, `raise-hand.png`, `thought.png`, `post-it.png`, `pinned-notes.png`, `sticky-note.png`, `hands_on.png`, `demo.png`

Plus a handful of vector versions: `hands_on.svg`, `lecturer.svg`, `people_group.svg`, `sticky_notes.svg`, `demo.svg`, `hospital_clinic.svg`.

These are **flat, two-color illustrative icons** — not pictograms — and they show up on welcome/break/objectives slides at around 120 px tall. They are decorative more than functional.

For **product UI** (the Deck Builder web app), the codebase uses **`lucide-react`** (you can spot it across `App.tsx`, `ContentLibrary.tsx`, etc — stroke icons at 16/20/24 px, `stroke-width: 1.5–2`). When mocking the web app, link Lucide from a CDN:

```html
<script type="module">
  // For static mockups, use icon URLs from https://unpkg.com/lucide-static@latest/icons/<name>.svg
  // For React mockups, import from https://esm.sh/lucide-react
</script>
```

**Emoji** — not in decks, not in methodology, not in the web app. Avoid.
...
**Substitution flags.** No icon substitutions in this system — the source icons live in `assets/icons/`. For new product UI work, use Lucide directly; if the design needs a workshop-style illustration that's not in the set, **ask the user** rather than drawing one in SVG.
