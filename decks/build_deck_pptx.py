...
GOV_EN = [
    ...
    {"type": "content", "title": "Four questions any government should ask", "blocks": [
        {"t": "**Could our data leave the country's control?**", "bullet": True},
        {"t": "**Could the wrong person get in?**", "bullet": True},
        {"t": "**Could data be intercepted on the way — or lost in a failure?**", "bullet": True},
        {"t": "**Could the AI expose what it sees?**", "bullet": True},
        {"t": "The next four slides take these one at a time: the risk, then what answers it."},
    ]},
    {"type": "image", "title": "Could our data leave the country's control?",
     "lead": "**The risk:** data flowing into a shared pool, or visible to another country. **The answer:** there is no shared pool — each country runs its own installation, and sharing is technically impossible.",
     "image": "diagrams/gov_country_isolation.png"},
    {"type": "content", "title": "Could the wrong person get in?", "blocks": [
        {"t": "**The risk:** a leaked password, or an insider reaching beyond their role. **The answer:** accounts, roles, and identity re-checked at every single request."},
    ...
    {"type": "image", "title": "Could data be intercepted — or lost?",
     "lead": "**The risk:** interception on the network, or a server failure taking the data with it. **The answer:** nothing readable leaves the platform, and automatic copies mean everything can be rebuilt.",
     "image": "diagrams/gov_security_layers.png"},
    {"type": "image", "title": "Could the AI expose what it sees?",
     "lead": "**The risk:** an assistant that sees too much. **The answer:** it is handed aggregated totals only — the numbers a user could already see, within that user's own permissions. **Never the underlying rows.**",
...
GOV_FR = [
    ...
    {"type": "content", "title": "Quatre questions que tout gouvernement devrait poser", "blocks": [
        {"t": "**Nos données peuvent-elles échapper au contrôle du pays ?**", "bullet": True},
        {"t": "**La mauvaise personne peut-elle entrer ?**", "bullet": True},
        {"t": "**Les données peuvent-elles être interceptées en chemin — ou perdues ?**", "bullet": True},
        {"t": "**L'IA peut-elle exposer ce qu'elle voit ?**", "bullet": True},
        {"t": "Les quatre diapositives suivantes les prennent une à une : le risque, puis ce qui y répond."},
    ]},
    {"type": "image", "title": "Nos données peuvent-elles échapper au contrôle du pays ?",
     "lead": "**Le risque :** des données versées dans un pot commun, ou visibles d'un autre pays. **La réponse :** il n'y a pas de pot commun — chaque pays fonctionne sur sa propre installation, et le partage est techniquement impossible.",
     "image": "diagrams_fr/gov_country_isolation.png"},
    {"type": "content", "title": "La mauvaise personne peut-elle entrer ?", "blocks": [
        {"t": "**Le risque :** un mot de passe qui fuite, ou un initié qui dépasse son rôle. **La réponse :** des comptes, des rôles, et une identité revérifiée à chaque requête."},
    ...
    {"type": "image", "title": "Les données peuvent-elles être interceptées — ou perdues ?",
     "lead": "**Le risque :** une interception sur le réseau, ou une panne de serveur qui emporte les données. **La réponse :** rien de lisible ne quitte la plateforme, et des copies automatiques permettent de tout reconstruire.",
     "image": "diagrams_fr/gov_security_layers.png"},
    {"type": "image", "title": "L'IA peut-elle exposer ce qu'elle voit ?",
     "lead": "**Le risque :** un assistant qui en voit trop. **La réponse :** il ne reçoit que des totaux agrégés — les chiffres qu'un utilisateur verrait déjà, dans la limite de ses propres permissions. **Jamais les lignes en dessous.**",
