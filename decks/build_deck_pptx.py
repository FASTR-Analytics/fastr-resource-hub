...
GOV_EN = [
    ...
    {"type": "image", "title": "What \"secure\" means for a platform like this",
     "lead": "Data security for a hosted platform spans five distinct dimensions. The following slides address each in turn: the risk, and the measures in place.",
     "image": "diagrams/gov_security_dimensions.png"},
    {"type": "content", "title": "The starting point: what the platform holds", "blocks": [
        ...
        {"t": "**Current scope, by design**: hosting individual-level records remains possible in future, subject to the additional safeguards set out in the technical annex", "bullet": True},
    ]},
    {"type": "image", "title": "Confidentiality — who can access the data",
     "lead": "The risk: unauthorized access, from outside or inside. The measures: named accounts and roles, identity re-checked on every request, encryption in transit.",
     "image": "diagrams/gov_security_layers.png"},
    {"type": "content", "title": "Integrity — can the figures be trusted", "blocks": [
        {"t": "The risk: figures altered without record, or analyses that cannot be reproduced. The measures: one source of truth, recorded imports, versioned results."},
    ...
    {"type": "content", "title": "Availability — what if a server fails", "blocks": [
        {"t": "The risk: hardware failure, accidental deletion, or a corrupted import. The measures: layered automatic backups and a tested recovery path."},
        {"t": "The **database is backed up every 30 minutes**, with a three-day rolling window", "bullet": True},
        {"t": "**Full installation snapshots** kept on a daily, weekly, and monthly cycle", "bullet": True},
        {"t": "An **entire country instance can be rebuilt** from these snapshots", "bullet": True},
        {"t": "Creating or restoring a backup requires **explicit permission** in addition to a valid login", "bullet": True},
    ]},
    {"type": "image", "title": "Sovereignty — the data remains the country's",
     "lead": "The risk: data flowing into a shared pool, or visible to another country. The measures: fully isolated installations, and full export at any time.",
     "image": "diagrams/gov_country_isolation.png"},
    {"type": "content", "title": "Accountability — every action attributable", "blocks": [
        {"t": "The risk: changes or access nobody can trace. The measures: every request tied to a named account, and operational logs across the platform."},
        {"t": "Every request runs under a **named, verified account** — no anonymous actions", "bullet": True},
        {"t": "**Imports are recorded** per indicator and month; **results packages are dated and versioned**", "bullet": True},
        {"t": "**Backup and restore actions are permission-gated** and attributable", "bullet": True},
    ...
    {"type": "image", "title": "Running costs",
     "lead": "Running costs fall into three lines: a **fixed hosting fee** for the country's server, **metered AI use** (the Anthropic API), and **shared platform maintenance**, which also covers external services such as the **Clerk login service**. The software itself is open source — there are no license or per-user fees.",
     "image": "diagrams/gov_cost_structure.png"},
    {"type": "content", "title": "The figures — planning estimates", "blocks": [
        {"t": "These are planning estimates at 2026 prices and current usage levels — **not a quote**."},
        {"t": "**Hosting** a country's instance: about **USD 500–800 per year** — larger countries toward the top", "bullet": True},
        {"t": "**AI use** (the Anthropic API): about **USD 350 per month** (~USD 4,000 per year) for a typical country; the largest countries several times this, growing with use", "bullet": True},
        {"t": "**Shared platform maintenance** (engineering, monitoring, backups, the Clerk login service): roughly **USD 7,000–10,000 per country per year**, falling as more countries join — an economy of scale forgone under self-hosting", "bullet": True},
        {"t": "**Optional support** (data refresh, quality checks, analysis): about **USD 5,000–15,000 per year**, depending on level", "bullet": True},
        {"t": "**Total: about USD 12,000 per year** — up to USD 20,000–30,000 with support; no other subscriptions", "bullet": True},
    ]},
    {"type": "content", "title": "Hosting — and what moving really takes", "blocks": [
        ...
        {"t": "**Migration is a structured project**: readiness assessment, server procurement, team training, a parallel run, then cutover — a **timeline of months, planned jointly**", "bullet": True},
        {"t": "**Self-hosting transfers the shared functions to the ministry**: maintenance, monitoring, and applying each update — deployed centrally to all countries at once today — with one-by-one support carrying additional cost", "bullet": True},
    ...
    {"type": "content", "title": "The essentials", "blocks": [
        {"t": "**Aggregated totals today** — no patient records; future individual-level hosting is conditional on added safeguards", "bullet": True},
        {"t": "**Security on five dimensions** — isolated installations, role-based access, recorded imports and versioned results, layered backups, full audit trails", "bullet": True},
        {"t": "**About USD 12,000 per year** to run — hosting, metered AI, shared maintenance; no license or per-user fees", "bullet": True},
        {"t": "**Country hosting by 2030** — the stated goal; a structured migration shifting updates and maintenance to the ministry, with likely additional per-country cost", "bullet": True},
    ]},
    ...
    {"type": "content", "title": "If patient-level data were hosted: the requirements", "blocks": [
        {"t": "The aggregated scope is a design decision, not a technical limit. Hosting individual-level data would be conditional on the following safeguards:"},
...
GOV_FR = [
    ...
    {"type": "image", "title": "Ce que « sécurisé » veut dire pour une telle plateforme",
     "lead": "La sécurité d'une plateforme de données recouvre cinq dimensions distinctes. Les diapositives suivantes traitent chacune d'elles : le risque, et les mesures en place.",
     "image": "diagrams_fr/gov_security_dimensions.png"},
    {"type": "content", "title": "Le point de départ : ce que contient la plateforme", "blocks": [
        ...
        {"t": "**Périmètre actuel, par conception** : l'hébergement de données individuelles reste possible, sous réserve des garanties de l'annexe technique", "bullet": True},
    ]},
    {"type": "image", "title": "Confidentialité — qui peut accéder aux données",
     "lead": "Le risque : un accès non autorisé, de l'extérieur comme de l'intérieur. Les mesures : des comptes nommés et des rôles, une identité revérifiée à chaque requête, le chiffrement en transit.",
     "image": "diagrams_fr/gov_security_layers.png"},
    {"type": "content", "title": "Intégrité — peut-on se fier aux chiffres", "blocks": [
        {"t": "Le risque : des chiffres modifiés sans trace, ou des analyses impossibles à reproduire. Les mesures : une source de vérité, des importations enregistrées, des résultats versionnés."},
    ...
    {"type": "content", "title": "Disponibilité — si un serveur tombe", "blocks": [
        {"t": "Le risque : une panne matérielle, une suppression accidentelle ou une importation corrompue. Les mesures : des sauvegardes automatiques en couches et un chemin de récupération éprouvé."},
        {"t": "La **base de données est sauvegardée toutes les 30 minutes**, fenêtre glissante de trois jours", "bullet": True},
        {"t": "Des **instantanés complets de l'installation** selon un cycle quotidien, hebdomadaire et mensuel", "bullet": True},
        {"t": "Une **instance pays entière peut être reconstruite** à partir de ces instantanés", "bullet": True},
        {"t": "Créer ou restaurer une sauvegarde exige une **permission explicite** en plus d'une connexion valide", "bullet": True},
    ]},
    {"type": "image", "title": "Souveraineté — les données restent celles du pays",
     "lead": "Le risque : des données versées dans un espace partagé, ou visibles d'un autre pays. Les mesures : des installations entièrement isolées, et l'export intégral à tout moment.",
     "image": "diagrams_fr/gov_country_isolation.png"},
    {"type": "content", "title": "Traçabilité — chaque action est attribuable", "blocks": [
        {"t": "Le risque : des modifications ou des accès que personne ne peut retracer. Les mesures : chaque requête liée à un compte nommé, et des journaux opérationnels sur toute la plateforme."},
        {"t": "Chaque requête s'exécute sous un **compte nommé et vérifié** — aucune action anonyme", "bullet": True},
        {"t": "**Importations enregistrées** par indicateur et par mois ; **paquets de résultats datés et versionnés**", "bullet": True},
        {"t": "**Sauvegardes et restaurations contrôlées par permission** et attribuables", "bullet": True},
    ...
    {"type": "image", "title": "Coûts de fonctionnement",
     "lead": "Les coûts de fonctionnement tiennent en trois lignes : un **hébergement fixe** pour le serveur du pays, l'**usage de l'IA au compteur** (l'API Anthropic), et la **maintenance partagée** de la plateforme, qui couvre aussi les services externes comme le **service de connexion Clerk**. Le logiciel lui-même est open source — ni licence ni frais par utilisateur.",
     "image": "diagrams_fr/gov_cost_structure.png"},
    {"type": "content", "title": "Les chiffres — estimations de planification", "blocks": [
        {"t": "Ce sont des estimations de planification, aux prix 2026 et à l'usage actuel — **non un devis**."},
        {"t": "**Hébergement** de l'instance : environ **500 à 800 USD par an** — les grands pays vers le haut de la fourchette", "bullet": True},
        {"t": "**Usage de l'IA** (l'API Anthropic) : environ **350 USD par mois** (~4 000 USD par an) pour un pays typique ; plusieurs fois plus pour les plus grands, en croissance avec l'usage", "bullet": True},
        {"t": "**Maintenance partagée** (ingénierie, supervision, sauvegardes, service de connexion Clerk) : environ **7 000 à 10 000 USD par pays et par an**, en baisse à mesure que des pays rejoignent — une économie d'échelle qui disparaît en auto-hébergement", "bullet": True},
        {"t": "**Appui optionnel** (actualisation des données, contrôles qualité, analyses) : environ **5 000 à 15 000 USD par an** selon le niveau", "bullet": True},
        {"t": "**Total : environ 12 000 USD par an** — jusqu'à 20 000–30 000 USD avec l'appui ; aucun autre abonnement", "bullet": True},
    ]},
    {"type": "content", "title": "Hébergement — et ce que migrer demande vraiment", "blocks": [
        ...
        {"t": "**La migration est un projet structuré** : évaluation de préparation, achat des serveurs, formation, fonctionnement en parallèle, puis bascule — un **calendrier en mois, planifié conjointement**", "bullet": True},
        {"t": "**L'auto-hébergement transfère au ministère les fonctions partagées** : maintenance, supervision et application de chaque mise à jour — déployée aujourd'hui centralement pour tous à la fois — avec un appui instance par instance à coût supplémentaire", "bullet": True},
    ...
    {"type": "content", "title": "L'essentiel", "blocks": [
        {"t": "**Des totaux agrégés aujourd'hui** — aucun dossier patient ; l'hébergement futur de données individuelles est conditionné à des garanties supplémentaires", "bullet": True},
        {"t": "**La sécurité sur cinq dimensions** — installations isolées, accès par rôles, importations enregistrées et résultats versionnés, sauvegardes en couches, journaux d'audit", "bullet": True},
        ...
        {"t": "**Hébergement par le pays d'ici 2030** — l'objectif affiché ; une migration structurée transférant mises à jour et maintenance au ministère, avec un coût supplémentaire probable par pays", "bullet": True},
    ]},
    ...
    {"type": "content", "title": "Si des données patients étaient hébergées : les exigences", "blocks": [
        {"t": "Le périmètre agrégé est une décision de conception, non une limite technique. L'hébergement de données individuelles serait conditionné aux garanties suivantes :"},
