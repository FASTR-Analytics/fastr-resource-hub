...
GOV_EN = [
    ...
    {"type": "image", "title": "Could our data leave the country's control?",
     "lead": "**Sovereignty.** The risk: data flowing into a shared pool, or visible to another country. The answer: each country runs a fully isolated installation — no shared components exist, cross-country access is technically impossible, and all data can be exported in full at any time.",
    ...
    {"type": "image", "title": "Could the AI expose what it sees?",
     "lead": "**Confidentiality and accountability.** The risk: an assistant that sees too much. The answer: the AI can only see what the signed-in user is permitted to see, and every question is logged. Today that means aggregated results; as facility-level analysis is introduced, **the same permission boundary applies**.",
    ...
    {"type": "content", "title": "Hosting — and what moving really takes", "blocks": [
        ...
        {"t": "**Self-hosting transfers the shared functions to the ministry**: maintenance, monitoring, upgrades and fixes become the ministry team's responsibility", "bullet": True},
        {"t": "**Update distribution changes too**: central deployment reaches all countries at once today; a self-hosted instance must receive and apply each update itself — one-by-one support carries additional cost", "bullet": True},
    ...
    {"type": "content", "title": "The essentials", "blocks": [
        ...
        {"t": "**Country hosting by 2030** — the stated transition goal; it shifts updates and maintenance to the ministry, with likely additional per-country cost", "bullet": True},
    ]},
...
GOV_FR = [
    ...
    {"type": "content", "title": "Ce que contient la plateforme", "blocks": [
        ...
        {"t": "**Périmètre actuel, par conception** : l'hébergement de données individuelles reste possible à l'avenir, sous réserve des garanties supplémentaires détaillées dans l'annexe technique", "bullet": True},
    ]},
    ...
    {"type": "image", "title": "Nos données peuvent-elles échapper au contrôle du pays ?",
     "lead": "**Souveraineté.** Le risque : des données versées dans un espace partagé, ou visibles d'un autre pays. La réponse : chaque pays fonctionne sur une installation entièrement isolée — aucun composant partagé, accès entre pays techniquement impossible, données exportables intégralement à tout moment.",
    ...
    {"type": "image", "title": "L'IA peut-elle exposer ce qu'elle voit ?",
     "lead": "**Confidentialité et traçabilité.** Le risque : un assistant qui en voit trop. La réponse : l'IA ne voit que ce que l'utilisateur connecté est autorisé à voir, chaque question journalisée. Aujourd'hui, des résultats agrégés ; l'analyse au niveau des établissements suivra **la même limite de permissions**.",
    ...
    {"type": "content", "title": "Hébergement — et ce que migrer demande vraiment", "blocks": [
        ...
        {"t": "**L'auto-hébergement transfère au ministère les fonctions partagées** : maintenance, supervision, mises à niveau et correctifs relèvent alors de l'équipe du ministère", "bullet": True},
        {"t": "**La distribution des mises à jour change aussi** : le déploiement central touche tous les pays à la fois aujourd'hui ; une instance auto-hébergée doit recevoir et appliquer chaque mise à jour elle-même — un appui instance par instance a un coût supplémentaire", "bullet": True},
    ...
    {"type": "content", "title": "L'essentiel", "blocks": [
        ...
        {"t": "**Hébergement par le pays d'ici 2030** — l'objectif de transition affiché ; il transfère mises à jour et maintenance au ministère, avec un coût supplémentaire probable par pays", "bullet": True},
    ]},
