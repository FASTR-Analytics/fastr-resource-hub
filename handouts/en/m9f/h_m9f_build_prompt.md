---
marp: true
theme: fastr-handout
paginate: true
class: redesign
footer: "FASTR · Prompting techniques"
---

<div class="brand-line"><span class="rule"></span><img src="../../../resources/logos/FASTR_Primary_01_FullName.png" alt="FASTR" height="28"></div>

<div class="setup-breadcrumb"><span class="step current">Build a clear prompt</span> <span class="arrow">→</span> <span class="step">Explore</span> <span class="arrow">→</span> <span class="step">Iterative vs single</span> <span class="arrow">→</span> <span class="step">Refine</span> <span class="arrow">→</span> <span class="step">PDF template</span> <span class="arrow">→</span> <span class="step">Verify output</span></div>

# Build a clear prompt

<p class="meta-line"><strong>Activity</strong> · <strong>Prompting techniques</strong> · <strong>~15 min</strong></p>

<div class="p1-grid">
<aside class="p1-sidebar">

<p class="sb-label">Before you start</p>

- ☐ You're signed in to the FASTR platform with the AI Assistant available
- ☐ You have a real workshop task in mind that you'd normally do (a chart, a summary, a slide for a meeting)

<p class="sb-label">Why it matters</p>

A clear prompt produces a useful answer; a vague prompt produces a generic one. The difference is **what you put in** — most of the value comes from a 30-second pre-flight before you type.

</aside>
<div class="p1-main">

## Six dimensions before you type

Pick a task you do regularly. Before writing your prompt, check it covers these six dimensions.

**1. Clear objective.** Is the task explicit (*interpret, summarize, compare, draft slides*) and the use case named (*performance review, donor update, training*)?

**2. Target audience.** Who's it for? At what technical level?

**3. Geography, period, scope.** National or subnational? Time period? Priority indicators or services?

**4. Interpretation guidance.** Should the AI describe, compare, or identify disruptions? Description only, or include implications?

**5. Output format.** Bullets or narrative? Slide-ready or prose? Length limit?

**6. Guardrails.** Tell the AI to stick to the data shown and flag uncertainties or data-quality issues.

</div>
</div>

---

<div class="brand-line"><span class="rule"></span><img src="../../../resources/logos/FASTR_Primary_01_FullName.png" alt="FASTR" height="28"></div>

## Now write your prompt

Use the six dimensions above to write a prompt that names **objective**, **audience**, **scope**, **interpretation guidance**, **format**, and **guardrails**. One paragraph is fine.

**Your prompt:**

<div class="answer-box" style="min-height: 170pt"></div>

## The golden rule

> Before you press Enter, ask yourself: *"Is what I expect back obvious?"* If not, add one more detail.

**Refining is normal too.** You don't have to get the prompt right in one go — you can keep talking to the AI to narrow it down. The six dimensions above just give you a stronger first message.

## What's next

Move on to **Explore with the AI Assistant** to try this prompt against the platform and learn how small changes to a prompt change the output.
